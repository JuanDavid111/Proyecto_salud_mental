...
def terapia_de_personas_con_problemas_mentales (data_health: pd.DataFrame, query_reporte_problemas_mentales:str):
    ...
    grafica_terapia_personas = px.pie(terapia_personas.query(f"Mental_Health_Condition == '{query_reporte_problemas_mentales}'"),values="Count_Consultatuion_History", names="Consultation_History", hole=.2)

    

    texto_analisis = lambda x: 'La gráfica sugiere que una proporción significativa de personas con problemas de salud mental no ha asistido a terapia, lo que indica una necesidad de abordar las barreras que impiden el acceso a estos servicios.' if  x == 'Yes' else 'La gráfica sugiere que un número significativo de personas sin problemas de salud mental diagnosticados asisten a terapia, lo que subraya la importancia de la terapia como una herramienta para el bienestar general.'

    return (grafica_terapia_personas, texto_analisis(query_reporte_problemas_mentales))